...
class TypeDataset(Dataset):
    def __init__(self, path, mode, lm='bert-base-uncased', column_length=64, multi_columns=True, window_size=1,duplicated=False):
        self.path = path
        self.mode = mode
            ...
            for t in tqdm(table):
                ...
                for i, column in enumerate(columns):
                    ...
                    if self.duplicated:
                        cell_vis=set()
                        for cell in column:
                            if cell[1][1].strip() in cell_vis:
                                continue
                            cell_vis.add(cell[1][1].strip())
            ...
            for t in tqdm(table):
                ...
                for i, column in enumerate(columns):
                    ...
                    if self.duplicated:
                        cell_vis=set()
                        for cell in column:
                            if str(cell).strip() in cell_vis:
                                continue
                            cell_vis.add(str(cell).strip())
